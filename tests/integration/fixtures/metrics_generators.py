...
class DeterministicAnomalies:
    # Counter resets occur at these offsets from start
    COUNTER_RESET_OFFSETS: List[timedelta] = field(
        default_factory=lambda: [
            timedelta(hours=6),  # First reset
            timedelta(hours=24),  # Second reset
        ]
    )

    # Data gaps: (offset, duration)
    ...
    SPIKE_OFFSETS: List[Tuple[timedelta, float]] = field(
        default_factory=lambda: [
            (timedelta(hours=18), 10.0),  # 10x spike
        ]
    )
    ...
    ZERO_VALUE_OFFSETS: List[timedelta] = field(
        default_factory=lambda: [
            timedelta(hours=30),  # Value drops to zero
        ]
    )
...
class MetricsDataGenerator:
    ...
    def latency_distribution(
        ...
        # Standard histogram buckets for latency (in ms)
        ...
        for _ in range(self.num_points):
            ...
            # Generate counts for each bucket
            counts = []
            total_requests = self.rng.randint(100, 1000)

            for i, bucket in enumerate(buckets):
                # Calculate cumulative probability up to this bucket
                if bucket <= 0:
                    prob = 0
                else:
    ...
    def generate_gauge_metrics(
        ...
        for service in services:
            for host in hosts:
                # Use diurnal pattern for CPU
                values = list(self.diurnal_pattern(20.0, 80.0))
        ...
        for service in services:
            for endpoint in endpoints:
                for status_code in status_codes:
                    # Base rate varies by status code
    ...
    def generate_connection_gauge_metrics(
        ...
        for service in services:
            for pool in pool_names:
                ...
                values = list(self.gauge_with_zero_drops(base, 0.2))
    ...
    def generate_gc_duration_metrics(
        ...
        metrics = []
        for service in services:
            values = list(self.spike_pattern(5.0, 10.0, 30))  # Spike every 30 minutes
...
# Convenience functions for quick metric generation
    ...
    for i, value in enumerate(values):
        ...
        metrics.append(
            Metrics(
                ...
                temporality=temporality,
                type_="Sum",
                is_monotonic=False,  # Key difference: non-monotonic
            )
        )
    return metrics
